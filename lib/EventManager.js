...
var EventManager = {
    ...
    observe: function observe(event) {
        ...
        observable = observables[event];

        originalSubscribe = observable.subscribe;

        // re-use the same subscribe function once it's being modified
        if ('observableSubscribe' !== observable.subscribe.name) {
            observable.subscribe = function observableSubscribe() {
                return _registerSubscription(event, originalSubscribe.apply(observable, arguments));
            };
        }
