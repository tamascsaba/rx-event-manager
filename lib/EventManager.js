 ...
 *
 * @private
 * @property mostRecentEventData
 * @type {Object}
 */
var mostRecentEventData = {};

/**
 * An Rx.Observable instance which emits sequences
 *
 * @private
 * @property eventManagerObservable
 * @type {Rx.Observable}
 */
var eventManagerObservable = Rx.Observable.create(function createObservable(o) {
    function handle(e) {
        try {
            o.onNext(e);
        } catch (ex) {
    ...
    eventManagerCore.subscribe(handle, o.onError.bind(o), o.onCompleted.bind(o));
...
var EventManager = {
    ...
    observe: function observe(event) {
        ...
        if ( ! observables[event]) {
            observables[event] = eventManagerObservable.
     ...
     */
    mostRecent: function mostRecent(event, onNext, onError, onCompleted) {
        var observable,
            observer,
            mostRecentData,
            subscription;
        ...
        mostRecentData = mostRecentEventData[event];

        if (mostRecentData && true === mostRecentData.hasValue) {
            observer.onNext(mostRecentData.value);
        }
    ...
    fire: function fire(event, data) {
        ...
        if ( ! mostRecentEventData[event]) {
            mostRecentEventData[event] = { hasValue: true };
        }

        mostRecentEventData[event].value = data;
    ...
    dispose: function dispose(event) {
        ...
        if (mostRecentEventData[event]) {
            delete mostRecentEventData[event];
        }
